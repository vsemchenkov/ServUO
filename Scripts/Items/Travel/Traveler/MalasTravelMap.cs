...
{
    public class MalasTravelGump : Gump
    {
        private int m_Page;
        private Mobile m_From;
        private MalasTravelMap m_TravelMap;
        ...
        public MalasTravelGump(Mobile from, int page, int x, int y, MalasTravelMap travelmap)
            : this(from, page, x, y, travelmap, 0, "")
        {
        }

        public MalasTravelGump(Mobile from, int page, int x, int y, MalasTravelMap travelmap, int detail)
            : this(from, page, x, y, travelmap, detail, "")
        {
        }

        public MalasTravelGump(Mobile from, int page, int x, int y, MalasTravelMap travelmap, int detail, string message)
            : base(x, y)
        {
            m_Page = page;
            m_From = from;
            ...
            if (m_TravelMap.Entries == null || m_TravelMap.Entries.Count <= 0)
            {
                ...
                m_From.CloseGump(typeof(MalasTravelGump));
                return;
            }
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            if (id < 100)
            {
                if (id == 4)
                {
                    m_From.SendGump(new MalasTravelGump(m_From, m_Page, X, Y, m_TravelMap));
                    m_From.SendGump(new MapTravelHelp(X, Y));
                }
            }
            else if (id < 10000)
            {
                m_From.SendGump(new MalasTravelGump(m_From, m_Page, X, Y, m_TravelMap, id));
            }
            else if (id < 20000)
            {
                    ...
                    if (MapTravelHelp.CanTravel(m_From, map, p) && (entry.Unlocked || !NonGM) && m_TravelMap.Parent == m_From.Backpack)
                    {
                        if (NonGM && CHARGE) // Do not charge GMs - they might complain...
                        {
                            ...
                            if (pack == null || (pack.ConsumeTotal(payTypes, payAmounts) > 0))
                            {
                                ...
                                m_From.SendGump(new MalasTravelGump(m_From, m_Page, X, Y, m_TravelMap, m_Detail, message));
                                return;
                            }
    ...
    public class MalasTravelMap : Item
    {
        ...
        [Constructable]
        public MalasTravelMap()
            : base(0x14EB)
        {
            Name = "Malas Travel Map";
        ...
        {
            get { return "Malas Travel Map"; }
        }

        public override void OnDoubleClick(Mobile from)
        {
            if (Parent == from.Backpack)
                from.SendGump(new MalasTravelGump(from, 1, 50, 60, this));
            else
        ...
        public MalasTravelMap(Serial serial)
            : base(serial)
        {
        }
        ...
        public List<MapTravelEntry> GetDefaultEntries()
        {
            ...
            // Malas Locations
            entries.Add(new MapTravelEntry(100, 1, "Luna", new Point3D(1015, 527, -65), Mal, 97, 119, 128, 124));
            entries.Add(new MapTravelEntry(101, 1, "Umbra", new Point3D(1997, 1386, -85), Mal, 268, 273, 309, 278));
            entries.Add(new MapTravelEntry(102, 1, "Orc Fort", new Point3D(912, 215, -90), Mal, 117, 241, 176, 247));
            entries.Add(new MapTravelEntry(103, 1, "Forgotten Pyramid", new Point3D(1853, 1797, -109), Mal, 162, 359, 273, 364));
            entries.Add(new MapTravelEntry(104, 1, "Doom", new Point3D(2368, 1267, -85), Mal, 340, 252, 374, 259));
            entries.Add(new MapTravelEntry(105, 1, "Labyrinth", new Point3D(1730, 981, -80), Mal, 203, 203, 260, 208));
            entries.Add(new MapTravelEntry(106, 1, "Grimswind Ruins", new Point3D(2203, 327, -90), Mal, 248, 78, 343, 83)); 

            return entries;
        }
    }

    public class MalasTravelMapPiece : Item
    {
        private Map m_Map;
        private int m_Index;
        ...
        [Constructable]
        public MalasTravelMapPiece()
            : this(Map.Malas)
        {
        }

        [Constructable]
        public MalasTravelMapPiece(Map map)
            : this(map, 0, "")
        {
        }

        [Constructable]
        public MalasTravelMapPiece(Map map, int index, string description)
            : base(0x14ED)
        {
            m_Map = map;
            m_Index = index;
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            MalasTravelMap map = pack.FindItemByType<MalasTravelMap>();
            if (map == null)
            {
                from.SendMessage("You must have a Malas Travel Map in your pack to use this map fragment.");
                return;
            }
        ...
        public MalasTravelMapPiece(Serial serial)
            : base(serial)
        {
        }
