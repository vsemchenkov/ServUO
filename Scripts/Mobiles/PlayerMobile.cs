    ...
    public partial class PlayerMobile : Mobile, IHonorTarget
    {
        ...
        public override void ToggleFlying()
        {
            ...
            if (!Flying)
            {
                ...
                {
                    LocalOverheadMessage(MessageType.Regular, 0x3B2,
                        1075124); // You must wait before casting that spell again.
                }
            }
        ...
        */
        ...
        private static void OnLogin(LoginEventArgs e)
        {
            ...
            if (((from.Map == Map.Trammel && from.Region.IsPartOf("Blackthorn Castle")) ||
                 PointsSystem.FellowshipData.Enabled && from.Region.IsPartOf("BlackthornDungeon") ||
                 from.Region.IsPartOf("Ver Lor Reg")) && from.Player && from.AccessLevel == AccessLevel.Player &&
                from.CharacterOut)
            {
        ...
        public override void OnSubItemRemoved(Item item)
        {
            if (Server.Engines.UOStore.UltimaStore.HasPendingItem(this))
                Timer.DelayCall<PlayerMobile>(TimeSpan.FromSeconds(1.5),
                    Server.Engines.UOStore.UltimaStore.CheckPendingItem, this);
        }
        ...
        public override void AggressiveAction(Mobile aggressor, bool criminal)
        {
                ...
                ((BaseCreature) aggressor).ControlMaster != this)
            {
                ...
                if (NotorietyHandlers.CheckAggressor(Aggressors, aggressor))
                {
                    ...
                    if (aggressiveMaster is PlayerMobile ||
        ...
        public override void GetContextMenuEntries(Mobile from, List<ContextMenuEntry> list)
        {
            ...
            if (from == this)
            {
                ...
                if (Alive)
                {
                    list.Add(new Server.Engines.Points.StatusPlayer(this));
                }
                ...
                {
                    var controller = Map == Map.Felucca
        ...
        private void SiegeBlessItem()
        {
            ...
            BeginTarget(2, false, TargetFlags.None, (from, targeted) => { Siege.TryBlessItem(this, targeted); });
        }
        ...
        private void ReleaseCoOwnership()
        {
            ...
            {
                SendGump(new WarningGump(1060635, 30720, 1062006, 32512, 420, 280,
                    new WarningGumpCallback(ClearCoOwners_Callback), house));
            }
        }
        ...
        public override bool CheckTrade(
            Mobile to, Item item, SecureTradeContainer cont, bool message, bool checkItems, int plusItems,
            int plusWeight)
        {
            int msgNum = 0;
            ...
            if ((this.Poison == null || this.Poison.Level < poison.Level) && ToleranceSpell.OnPoisonApplied(this))
            {
                ...
                if (poison == null || poison.Level <= 0)
                {
                    PrivateOverheadMessage(MessageType.Regular, 0x3F, 1053092,
                        this.NetState); // * You feel yourself resisting the effects of the poison *
                    return ApplyPoisonResult.Immune;
                }
            }
        ...
        public int RealLuck
        {
            get
            {
                int facetBonus = !Siege.SiegeShard && this.Map == Map.Felucca
                    ? RandomItemGenerator.FeluccaLuckBonus
                    : 0;
        ...
        private bool m_PvPTitle = true;

        [CommandProperty(AccessLevel.Owner)]
        public bool PvPTitle
        {
            get { return m_PvPTitle; }
            set
            {
                m_PvPTitle = value;
                InvalidateProperties();
        ...
        public override void GetProperties(ObjectPropertyList list)
        {
            ...
            if (this.PvPTitle)
            {
                if (this.PointSystems.PvPPoints == 0)
                {
                    list.Add($"Newbie");
                    m_PlayerLevel = 1;
                }
                else if (this.PointSystems.PvPPoints >= 1 && this.PointSystems.PvPPoints <= 14)
                {
                    list.Add($"Recruit");
                    m_PlayerLevel = 2;
                }
                else if (this.PointSystems.PvPPoints >= 15 && this.PointSystems.PvPPoints <= 44)
                {
                    list.Add($"Scout");
                    m_PlayerLevel = 3;
                }
                else if (this.PointSystems.PvPPoints >= 45 && this.PointSystems.PvPPoints <= 89)
                {
                    list.Add($"Corporal");
                    m_PlayerLevel = 4;
                }
                else if (this.PointSystems.PvPPoints >= 90 && this.PointSystems.PvPPoints <= 149)
                {
                    list.Add($"Sergeant");
                    m_PlayerLevel = 5;
                }
                else if (this.PointSystems.PvPPoints >= 150 && this.PointSystems.PvPPoints <= 224)
                {
                    list.Add($"Knight");
                    m_PlayerLevel = 6;
                }
                else if (this.PointSystems.PvPPoints >= 225 && this.PointSystems.PvPPoints <= 314)
                {
                    list.Add($"Commander");
                    m_PlayerLevel = 7;
                }
                else if (this.PointSystems.PvPPoints >= 315 && this.PointSystems.PvPPoints <= 419)
                {
                    list.Add($"Major");
                    m_PlayerLevel = 8;
                }
                else if (this.PointSystems.PvPPoints >= 420 && this.PointSystems.PvPPoints <= 539)
                {
                    list.Add($"Colonel");
                    m_PlayerLevel = 9;
                }
                else if (this.PointSystems.PvPPoints >= 540 && this.PointSystems.PvPPoints <= 674)
                {
                    list.Add($"Warlord");
                    m_PlayerLevel = 10;
                }
                else if (this.PointSystems.PvPPoints >= 675 && this.PointSystems.PvPPoints <= 999)
                {
                    list.Add($"General");
                    m_PlayerLevel = 11;
                }
                else if (this.PointSystems.PvPPoints >= 1000)
                {
                    list.Add($"Marshal");
                    m_PlayerLevel = 12;
                }
            ...
            {
                Server.Engines.VvV.VvVPlayerEntry entry = Server.Engines.Points.PointsSystem.ViceVsVirtue
                    .GetPlayerEntry<Server.Engines.VvV.VvVPlayerEntry>(this);
        ...
        private bool m_PvPPostSuffix = true;

        [CommandProperty(AccessLevel.Owner)]
        public bool PvPPostSuffix
        {
            get { return m_PvPPostSuffix; }
            set
            {
                m_PvPPostSuffix = value;
                InvalidateProperties();
            }
        }
        public override void AddNameProperties(ObjectPropertyList list)
        {
            ...
            bool vvv = Server.Engines.VvV.ViceVsVirtueSystem.IsVvV(this) &&
                       (ViceVsVirtueSystem.EnhancedRules || this.Map == ViceVsVirtueSystem.Facet);
            ...
            list.Add(1050045, "{0} \t{1}\t {2}\t {3}", prefix, name, suffix, postsuffix); // ~1_PREFIX~~2_NAME~~3_SUFFIX~
        ...
        public  string ApplyNamePostSuffix(string postsuffix)
        {
            if (Young)
            {
                if (postsuffix.Length == 0)
                {
                    postsuffix = "(Young)";
                }
                else
                {
                    postsuffix = String.Concat(postsuffix, " (Young)");
                }
            }

            return base.ApplyNamePostSuffix(postsuffix);
        }
        ...
        private static readonly Point3D[] m_TrammelDeathDestinations = new[]
        {
            ...
            new Point3D(1412, 3793, 0), new Point3D(3688, 2232, 20), new Point3D(2578, 604, 0),
            new Point3D(4397, 1089, 0),
            new Point3D(5741, 3218, -2), new Point3D(2996, 3441, 15), new Point3D(624, 2225, 0),
        ...
        private static readonly Point3D[] m_IlshenarDeathDestinations = new[]
        {
            ...
            new Point3D(986, 1011, -32), new Point3D(1175, 1287, -30), new Point3D(1533, 1341, -3),
